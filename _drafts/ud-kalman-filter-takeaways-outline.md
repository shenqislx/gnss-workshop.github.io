...
> 状态：草稿提纲，仅用于后续扩写；当前文件位于 `_drafts`，并提交在独立分支 `agent/udkf-takeaways-outline`，不进入 Pages 发布分支。
...
- Agee–Turner rank-one 的“rank-one”到底指什么。
...
- Bierman 标量 UD measurement update 本质上采用针对 Kalman 负 rank-one 更新重构的稳定递推；现代资料常将其核心归入 modified Agee–Turner 一类。
...
- 状态间相关性不仅存于某一个 \(U_{ij}\)，而是分散编码在 \(U\) 的整行后续元素中。
...
## 四、标量 measurement update 中新息方差 \(S\)
...
实际需要的 rank-one 次数等于 \(Q\) 的有效秩，而不一定等于状态维数。
...
## 十三、Agee–Turner、Carlson、Bierman 的历史顺序
...
12. **算法名称不如具体公式重要：应检查分解约定、循环方向、α 递推和原地覆盖顺序。**
